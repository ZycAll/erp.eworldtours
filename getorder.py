...
class GetOrder:
    ...
    def get(self,filename,payload):
        response1 = requests.post(self.url1, headers=self.headers, params=payload)
        self.data["file"]= response1.text
        response2 = requests.post(self.url2, headers=self.headers, data=self.data)
        with open(filename, 'wb') as f:
            f.write(response2.content)
        print(f"文件已保存为: {filename}")
if __name__ == '__main__':
    Order = GetOrder()
    #Order.get('excel_xlsx/AllOrder-Eworld.xlsx',payload=Order.payload1)
    #Order.get('excel_xlsx/Order2-Eworld.xlsx', payload=Order.payload2)
    #Order.get('excel_xlsx/Order3-Eworld.xlsx', payload=Order.payload3)
    Order.headers["Cookie"] = "_gcl_au=1.1.260138684.1757388165; _ga=GA1.1.1543263768.1757388196; _ga_ME5JMBJSZJ=GS2.1.s1757466017$o4$g0$t1757467542$j60$l0$h0; ASP.NET_SessionId=k4fucpewapr3x45smv3kurjn; uinfo=yunlin=83D5C5C45BC659D4F247EAFFA5F9D8A7CC997023166BB636; lan=cn; TESTCookieName=TEsTCookievalue"
    Order.headers["Referer"] = "https://erp.eworldtours.com/plugins/J2Travel.order/order/orderlist.aspx?lm=gty&type=nb&otype=dx&modelid=5856bed2-7d97-4a9f-932a-b6e77e08fe36&lan=cn"
    Order.get('excel_xlsx/AllOrder-Tinma.xlsx', payload=Order.payload4)
    Order.get('excel_xlsx/SupplyOrder-Tinma.xlsx', payload=Order.payload5)
    Order.get('excel_xlsx/DistributionOrder-Tinma.xlsx', payload=Order.payload6)
